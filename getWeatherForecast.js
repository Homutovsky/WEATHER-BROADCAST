...
export const getWeatherForecast = function (Latitude, Longitude) {
    ...
    request(weatherQuery.getRequestUrl()).then((date) => {
        ...
        window.sessionStorage.setItem("weatherDate", JSON.stringify(date));
        ...
        temperature.textContent = `${Math.round(
            extraInfromation.main.temp
        )}  °C`;
        humidity.textContent = `влажность  ${extraInfromation.main.humidity} %`;
        pressure.textContent = `давление  ${extraInfromation.main.pressure * 0.75
            } мм рт ст`;
        ...
        let tomorrowDate = `${newDate.getFullYear()}-${currentMonth}-${"0" + (+currentDate + 1)} 15:00:00`;

        let index = objectWithArr.find((element, ind) => element.dt_txt === tomorrowDate ? ind : -1)
        console.log(index)
        ...
        function createCardWeather(
            ...
            weatherDay.textContent = (
                "" + new Date(newDate.getTime() + 86400000 * nomber)
            ).split(" ")[0];
            weatherWeather.textContent =
                objectWithArr[index].weather[0].description;
            weatherTemperature.textContent = `${Math.round(
                objectWithArr[index].main.temp
            )} °C`;
        }
        createCardWeather(
            weatherDateToday,
